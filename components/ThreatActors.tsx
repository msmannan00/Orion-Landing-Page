    ...
    <div className="py-12 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row justify-between items-end gap-10 mb-12 border-b border-white/5 pb-12">
        <div className="max-w-3xl space-y-4">
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold text-white tracking-tight">Threat Inventory.</h2>
        ...
        <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-4">
          ...
          <div className="px-5 py-3 bg-blue-500/10 border border-blue-500/20 rounded-xl text-[10px] font-bold text-blue-400 uppercase tracking-widest whitespace-nowrap">
      ...
      <div className="flex flex-wrap gap-3 mb-12">
        <button 
          ...
          className={`px-5 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border ${!activeCategory ? 'bg-blue-600 border-blue-500 text-white' : 'bg-white/[0.03] border-white/10 text-white/40 hover:text-white'}`}
        >
          All Sources
        </button>
        {categories.map(cat => (
          <button 
            ...
            className={`px-5 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border ${activeCategory === cat ? 'bg-blue-600 border-blue-500 text-white' : 'bg-white/[0.03] border-white/10 text-white/40 hover:text-white'}`}
          >
            {cat}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredActors.map((actor, idx) => (
          <div key={idx} className="glass-card group p-5 rounded-2xl border border-white/10 hover:border-blue-500/40 transition-all flex flex-col h-full hover:bg-white/[0.05]">
            <div className="flex justify-between items-start mb-5">
              <div className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider border ${
                actor.category === 'Ransomware (RaaS)' ? 'bg-red-500/10 border-red-500/30 text-red-500' :
                actor.category === 'Intelligence Aggregator' ? 'bg-blue-500/10 border-blue-500/30 text-blue-500' :
                'bg-purple-500/10 border-purple-500/30 text-purple-500'
              }`}>
                {actor.category}
              </div>
              <div className="p-2 rounded-lg bg-white/[0.03] text-white/10 group-hover:text-blue-500 transition-colors">
            ...
            <p className="text-[11px] text-white/50 leading-relaxed mb-4 font-medium italic line-clamp-2">
              {actor.description}
            </p>

            <div className="space-y-3 mb-6 flex-grow">
              <div className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5 group-hover:bg-blue-500/[0.02] transition-colors">
              ...
              <div className="flex flex-wrap gap-1.5">
                {actor.tags.map(tag => (
                  <span key={tag} className="px-2 py-0.5 rounded bg-white/[0.03] text-[8px] font-mono text-white/20 uppercase tracking-tighter border border-white/5">
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            <a 
              ...
              className="mt-auto flex items-center justify-between px-3 py-2.5 bg-white/[0.02] border border-white/5 rounded-xl group-hover:border-blue-500/20 group-hover:bg-blue-500/5 transition-all overflow-hidden"
            >
              <div className="flex items-center gap-2.5 overflow-hidden">
                <Globe className="w-3 h-3 text-white/20 group-hover:text-blue-500 transition-colors" />
                <span className="text-[9px] font-mono text-white/20 group-hover:text-white transition-colors truncate">
                  {actor.url.replace('http://', '').replace('https://', '')}
                </span>
              </div>
              <ExternalLink className="w-3 h-3 text-white/20 group-hover:text-blue-500 shrink-0 ml-3" />
            </a>
          </div>
        ))}
      </div>
      ...
      {filteredActors.length === 0 && (
        <div className="py-32 flex flex-col items-center justify-center text-center space-y-6">
           ...
           <div className="space-y-2">
             <h4 className="text-xl font-bold text-white uppercase tracking-widest">No Adversaries Found</h4>
             <p className="text-white/30 text-sm max-w-sm">No entities match your filter or search criteria.</p>
           </div>
           <button 
